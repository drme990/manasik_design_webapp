...
export default function EditorPage() {
    ...
    // This route is for TEMPLATES only (kind='booking_template'). If the
    // loaded project is a design, redirect to /editor/d/[id].
    useEffect(() => {
        storeGetProject(id).then((p) => {
            if (p && p.kind !== 'booking_template') {
                router.replace(`/editor/d/${id}`);
                return;
            }
            setProject(p);
    ...
    // Change font size only — the box stays the same size. The text
    // reflows within the existing box. Use the resize handles to scale
    // both the box and font together.
